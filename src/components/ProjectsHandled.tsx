...
const ProjectsHandled: React.FC = () => {
  return (
    <div>
      <div className="relative w-[100%] h-[500px] flex justify-center items-center">
        <img
            ...
            {Content.map((item, index) => (
              <div className="h-[300px] w-[400px] flex justify-center items-center">
                <div className="flex flex-col justify-center items-start ml-10">
                  <div key={index} className="text-[60px] font-semibold text-primary">
                    {item.count}
                  </div>
                  <div className="border-2 border-primary w-[80px]"></div>
                  <h2 className="text-[30px] font-bold mt-5 text-primary">{item.text}</h2>
                </div>
              </div>
            ))}
          </div>
